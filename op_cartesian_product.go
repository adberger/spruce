...
func (CartesianProductOperator) Run(ev *Evaluator, args []*Expr) (*Response, error) {
	log.DEBUG("running (( cartesian-product ... )) operation at $.%s", ev.Here)
	defer log.DEBUG("done with (( cartesian-product ... )) operation at $%s\n", ev.Here)
	...
	for i, arg := range args {
		...
		case Reference:
			...
			case []interface{}:
				...
				log.DEBUG("     [%d]: resolved to a list; verifying", i)
				for j, sub := range s.([]interface{}) {
					if _, ok := sub.([]interface{}); ok {
						log.DEBUG("       list[%d]: list item is itself a list; error!", j)
						...
						log.DEBUG("       list[%d]: list item is a map; error!", j)
						return nil, fmt.Errorf("cartesian-product operator can only operate on lists of scalar values")

					}
					log.DEBUG("       list[%d]: list item is a scalar: %v", j, sub)
			...
			default:
				log.DEBUG("     [%d]: resolved to a scalar; appending", i)
	...
	switch len(args) {
	case 0:
		log.DEBUG("  no arguments supplied to (( cartesian-product ... )) operation.  oops.")
	...
	default:
		log.DEBUG("  called with more than one arguments; combining into a single list of strings")
