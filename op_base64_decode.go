...
func (Base64DecodeOperator) Run(ev *Evaluator, args []*Expr) (*Response, error) {
	log.DEBUG("running (( base64-decode ... )) operation at $.%s", ev.Here)
	defer log.DEBUG("done with (( base64-decode ... )) operation at $%s\n", ev.Here)
	...
	if decoded, err := base64.StdEncoding.DecodeString(contents); err == nil {
		log.DEBUG("  resolved (( base64-decode ... )) operation to the string:\n    \"%s\"", string(decoded))
		return &Response{
			Type:  Replace,
